        ...
        struct device_attribute *attr, char *buf)
{
    ...
    /* irq_to_desc() is not exported to modules on mainline kernels, and
     * struct irq_desc internals aren't module-accessible. Report just the
     * IRQ number to avoid the dependency. */
    return scnprintf(buf, PAGE_SIZE, "IRQ num: %d\n", cts_data->pdata->irq);
}
...
static struct device_attribute touchscreen_attributes[] = {
	...
	/* debug_log_level_{show,store} are compiled under this same guard */
